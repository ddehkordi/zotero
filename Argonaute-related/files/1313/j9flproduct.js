...
if(score==1){score=0;}
return score;};functions.updateIndicator=function($indicator,value,min,max,target){var $lang_selector=$(".literatumLocaleChanger .language");var currentLanguageValue=$("html").attr("lang");var too__short="Too Short";var strong="Strong";var weak="Weak";var too__long="Too Long";if($lang_selector.length>0){if($lang_selector.text()==="ESP"||$lang_selector.text()===""){too__short="Too Short";strong="Strong";weak="Weak";too__long="Too Long";}
else if($lang_selector.text()==="DEU"||$lang_selector.text()===""){too__short="Zu kurz";strong="Schwach";weak="Schwach";too__long="Zu lang";}
else{too__short="Muy corta";strong="Fuerte";weak="Débil";too__long="Muy larga";}}
else{if(currentLanguageValue==="de"){too__short="Zu kurz";strong="Schwach";weak="Schwach";too__long="Zu lang";}}
$indicator.removeClass('too-short too-long weak strong very-strong');var pass_selector=$('.pass-strength-popup');pass_selector.removeClass('too-short too-long weak strong very-strong');if(!value){pass_selector.find('.strength').text(too__short).hide();pass_selector.find('.strength').text(too__long).hide();pass_selector.removeClass('weak');pass_selector.removeClass('strong');return;}
var length=value.trim().length;if(length<min){$indicator.addClass('too-short');pass_selector.addClass('too-short');pass_selector.find('.strength').text(too__short).show();pass_selector.removeClass('weak');pass_selector.removeClass('strong');return;}
if(length>max){$indicator.addClass('too-long');pass_selector.addClass('too-long');pass_selector.find('.strength').text(too__long).show();pass_selector.removeClass('weak');pass_selector.removeClass('strong');pass_selector.addClass('too-long');$(".pass-strength-popup").css("display","block");return;}
...
setTimeout(function(){$(".item__removal__popup__text").fadeOut("slow");},4000);});$(document).ready(function(){$(".js__mail_verification_widget input").on("keyup",function(){if($(this).val().trim().length>0){$(this).closest("form").find(".form-btn").addClass("blue-subb-btn");$(this).closest("form").find(".form-btn").prop('disabled',false);}else{$(this).closest("form").find(".form-btn").removeClass("blue-subb-btn");$(this).closest("form").find(".form-btn").prop('disabled',true);}});$(".raa-modal-dialog .raa-modal-dialog-cncl").on("click",function(e){e.preventDefault();$(this).closest(".raa-modal-dialog").hide();});$(".raa-modal-dialog.enabled").show();var lower=/[a-z]/;var upper=/[A-Z]/;var special=/[!@#\$%\^\&*\)\(+=._-]/;var numeric=/[0-9]/;function translate_msg(lang,pswd_l,pswd_r,isLegacy){var message;if(isLegacy){if(lang==="ESP"||lang===""){if(pswd_r===2||pswd_r===4||pswd_r===5){message="Your password must have "+pswd_l+" characters or more:";}
else{message="Your password must have "+pswd_l+" characters or more and contain "+pswd_r+" of the following:";}}
else{if(pswd_r===2||pswd_r===4||pswd_r===5){message="Su contraseña debe tener: "+pswd_l+" caracteres o más:";}
else{message="Su contraseña debe tener: "+pswd_l+" caracteres o más y debe cumplir "+pswd_r+" de los siguientes requisitos:";}}}
else{if(lang==="es"){if(pswd_r===2||pswd_r===4||pswd_r===5){message="Su contraseña debe tener: "+pswd_l+" caracteres o más:";}
else{message="Su contraseña debe tener: "+pswd_l+" caracteres o más y debe cumplir "+pswd_r+" de los siguientes requisitos:";}}
else if(lang==='de'){if(pswd_r===2||pswd_r===4||pswd_r===5){message="Ihr Passwort muss aus mindestens "+pswd_l+" Zeichen bestehen";}
else{message="Ihr Passwort muss aus mindestens "+pswd_l+" Zeichen bestehen und "+pswd_r+" oder mehr der folgenden Elemente enthalten:";}}
else{if(pswd_r===2||pswd_r===4||pswd_r===5){message="Your password must have "+pswd_l+" characters or more:";}
else{message="Your password must have "+pswd_l+" characters or more and contain "+pswd_r+" of the following:";}}}
return message;}
function strength_list(lang,pswd,isLegacy){var list_items_strength;var strength;var separator='';if(isLegacy){if(lang==="ESP"||lang===""){strength="Too short";if(pswd===2){separator='<li class="separator"><span>or an</span></li>'}
else{separator="an "}
list_items_strength='<ul>'+'<li id="letter" class="invalid"><span>a lower case character</span></li>'+'<li id="capital" class="invalid"><span>'+separator+'upper case character</span></li>'+'<li id="special" class="invalid"><span>a special character</span></li>'+'<li id="number" class="invalid"><span>or a digit</span></li>'+'</ul><span class="strength">'+strength+'</span>';}
else{strength="Muy corta";if(pswd===2){separator='<li class="separator"><span>o</span></li>'}
else{separator='<li class="separator"><span>, un</span></li>'}
list_items_strength='<ul>'+'<li id="letter" class="invalid"><span>una minúscula</span></li>'+'<li id="capital" class="invalid"><span>una mayúscula</span></li>'+'<li id="special" class="invalid"><span>un carácter especial</span></li>'+'<li id="number" class="invalid"><span> o un número.</span></li>'+'</ul><span class="strength">'+strength+'</span>';}}
else{if(lang==="es"){strength="Muy corta";if(pswd===2){separator='<li class="separator"><span>o</span></li>'}
else{separator='<li class="separator"><span>, un</span></li>'}
list_items_strength='<ul>'+'<li id="letter" class="invalid"><span>una minúscula</span></li>'+'<li id="capital" class="invalid"><span>una mayúscula</span></li>'+'<li id="special" class="invalid"><span>un carácter especial</span></li>'+'<li id="number" class="invalid"><span> o un número.</span></li>'+'</ul><span class="strength">'+strength+'</span>';}
else if(lang==="de"){strength="Zu kurz";list_items_strength='<ul>'+'<li id="letter" class="invalid"><span>Kleinbuchstaben</span></li>'+'<li id="capital" class="invalid"><span>'+'Großbuchstaben</span></li>'+'<li id="special" class="invalid"><span>Sonderzeichen</span></li>'+'<li id="number" class="invalid"><span>Ziffern</span></li>'+'</ul><span class="strength">'+strength+'</span>';}
else{strength="Too short";if(pswd===2){separator='<li class="separator"><span>or an</span></li>'}
else{separator="an "}
list_items_strength='<ul>'+'<li id="letter" class="invalid"><span>a lower case character</span></li>'+'<li id="capital" class="invalid"><span>'+separator+'upper case character</span></li>'+'<li id="special" class="invalid"><span>a special character</span></li>'+'<li id="number" class="invalid"><span>or a digit</span></li>'+'</ul><span class="strength">'+strength+'</span>';}}
return list_items_strength;}
if($('.pass-hint')){$('.pass-hint').on('keyup input focus change',function(){var pswd=$(this).val();var $indicator=$('.pass-hint').siblings('.password-strength-indicator');var pswd_req=$indicator.data('strength');var pswd_length=$indicator.data('min');var pswd_max=$indicator.data('max');var $lang_selector=$(".literatumLocaleChanger .language");var currentLanguageValue=$("html").attr("lang");var isLegacy=$lang_selector.length>0;var currentLanguage=isLegacy?$lang_selector.text():currentLanguageValue;if(!pswd_req){pswd_req=3;}
var validator='<div id="pswd_info" class="pass-strength-popup js__pswd_info">'+'<h4 id="length">'+translate_msg(currentLanguage,pswd_length,pswd_req,isLegacy)+'</h4>'+strength_list(currentLanguage,pswd_req,isLegacy)+'</div>';if(!$('.js__pswd_info').length){$(this).closest(".input-group").append(validator);}
...
if((e.keyCode||e.which)===tabKey){if(!revers){tabEvent();}else{tabRevers();}}});$('.show-login:not(.userLoginBar__controller)').click(function(event){$popups.addClass('hidden');$popup.removeClass('hidden');$('body').addClass('noscroll');event.preventDefault();});$('.redicrect__close').click(function(){$('.redirect__modal .close').click();});$popup.on('click','.close',function(e){e.preventDefault();$('body').removeClass('noscroll');$popup.addClass('hidden');$eye.addClass('hidden icon-eye').removeClass('icon-eye-blocked');$submit.attr('disabled',true);$remember.attr('checked',false);$login.val('');$password.val('');$message.html('');});$(document).on('keyup',$popup,function(e){if(e.keyCode==27){$popup.find('.close').trigger("click");}
if(e.keyCode===shift){revers=false;}});function tabEvent(){$close.off();lastItem.on('focusout',function(){$close.focus();});}
...
$(document).ready(function(){var $registrationForm=$('[action="/action/registration"]');if($registrationForm.length){$registrationForm.find('.label.error').first().next('input').focus();}});});
...
var pattern=/^([\w-+]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,9}(?:\.[a-z]{2})?)$/i;var change=function(event){var code=event.keyCode?event.keyCode:event.which;if(code==13){return;}
...
$original.removeClass('hidden');$content.slideDown('fast');$content.find(":focusable").first().focus();event.preventDefault();});$drawer.on('click','.cancel',function(event){$content.slideUp('fast');$message.html('');$email.val('');$submit.attr('disabled',true);$drawer.addClass('hidden');event.preventDefault();$('.login-popup').find(":focusable").first().focus();if($drawer.hasClass('hidden')){$('a').attr('tabindex','0');$('input').attr('tabindex','0');if($('.top-header-menu-btn'))
$('.top-header-menu-btn').attr("tabindex","0");}});$drawer.on('keyup',function(e){if(e.keyCode==27){$drawer.find('.cancel').trigger("click");}});$form.submit(function(event){event.preventDefault();if(!$email.val()){return;}
...
$(function(){var $drawers=$('.top-drawer');var $drawer=$('.change-password-drawer');var $content=$drawer.find('.content');var $form=$content.find('form');var $old=$form.find('.old');var $new=$form.find('.new');var $message=$form.find('.message');var $submit=$form.find('.submit');var $original=$content.find('.form');var $success=$content.find('.success-template');var $indicator=$content.find('.password-strength-indicator');var $eye=$content.find('.password-eye-icon');$('.show-change-password').click(function(event){$drawers.addClass('hidden');$drawer.removeClass('hidden');$success.addClass('hidden');$original.removeClass('hidden');$content.slideDown('fast');$content.find("input:focusable").first().focus();event.preventDefault();});$drawer.on('click','.cancel',function(event){$content.slideUp('fast');$old.attr('type','password').val('');$new.attr('type','password').val('');$indicator.removeClass('too-short too-long weak medium strong very-strong');$eye.addClass('hidden icon-eye').removeClass('icon-eye-blocked');$message.text('');$submit.attr('disabled',true);$drawer.addClass('hidden');if($drawer.hasClass('hidden')){$('a').attr('tabindex','0');$('input').attr('tabindex','0');if($('.top-header-menu-btn'))
